...
import allow.simulator.collective.adaptation.ensemble.Role;
...
public class DemoManagementSystem {
	...
	File f = new File(classloader.getResource(REPO_PATH + File.separator + type + ".xml").getFile());
	...
	for (int i = 0; i < cap.getEnsembles().size(); i++) {
	    CollectiveAdaptationEnsemble cae = cap.getEnsembles().get(i);
	    if (cae.getEnsembleName().equals(ei.getName()) && cae.getRoles() != null && !cae.getRoles().isEmpty()) {
		// ensemble found
		for (int j = 0; j < cae.getRoles().size(); j++) {

		    String roleName = cae.getRoles().get(j).getRole();
		    if (roleName.equals(cap.getTarget())) {
			// target found
			// System.out.println(roleName);
			// find the right role in the ensemble parsed from file
			for (int k = 0; k < ei.getRole().size(); k++) {
			    Role rm = ei.getRole().get(k);
			    if (rm.getSolver().size() > 1 && (rm.getType().contains("RoutePassenger")))

				rm.setType(cap.getTarget());
			}
		    } else if (roleName.equals(cap.getStartingRole())) {
			for (int k = 0; k < ei.getRole().size(); k++) {
			    Role rm = ei.getRole().get(k);
			    if ((rm.getType().contains("FlexibusDriver")))
